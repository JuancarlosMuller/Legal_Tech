...
import documento from '!!raw-loader!../../img/documento.txt';




const textData = documento;
...
function CartaAviso_Form() {
    ...
    const generateDocument = () => {
        // Copiar el contenido decodificado del TXT
        let outputDocument = textData;

        // Reemplazar los marcadores de posición con datos del formulario
        for (const key in formData) {
            if (formData.hasOwnProperty(key)) {
                const value = formData[key];
                const regex = new RegExp(`<<\\s*${key}\\s*>>`, 'g');
                outputDocument = outputDocument.replace(regex, value);
            }
        }

        // Descargar el documento generado como un archivo de texto (TXT)
        const blob = new Blob([outputDocument], { type: 'text/plain' });
        saveAs(blob, 'DocumentoCompletado.txt');
    };
